...
class EntityInfo(dict):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__()
        data: dict[str, Any] = {}
        if len(args) == 1 and isinstance(args[0], (dict, Mapping)):
            data.update(args[0])
...
class UserInfo(EntityInfo):
    ...
    def __setattr__(self, key: str, value: Any) -> None:
        if key == "orgs" and value is not None:
            value = [org if isinstance(org, UserOrgInfo) else UserOrgInfo(org) for org in value]
        super().__setattr__(key, value)
        if key == "is_pro":
            dict.__setitem__(self, "is_pro", value)
            dict.__setitem__(self, "isPro", value)
        elif key == "orgs":
            dict.__setitem__(self, "orgs", value if value is not None else [])
